...
const Progress = () => {
  ...
  const [user, setUser] = useState({ fullname: "" }); // ✅ For user name
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE}/progress/stats?days=${selectedPeriod}`,
        { headers: getAuthHeaders() }
      );
  ...
  const fetchWeeklyData = async () => {
    try {
      const response = await axios.get(`${API_BASE}/progress/weekly`, {
        headers: getAuthHeaders(),
      });
  ...
  useEffect(() => {
    fetchUserProfile(); // ✅ Fetch user name on mount
    fetchStats();
    fetchWeeklyData();
  }, [selectedPeriod]);
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">
              {user.fullname ? `Hello, ${user.fullname}` : "My Progress"}
            </h1>
