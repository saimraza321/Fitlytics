...
const MealPlan = () => {
  ...
  // ✅ NEW: Open modal for adding (reset form)
  ...
  // ✅ NEW: Open modal for editing (fill existing data)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        // Edit existing meal
        await axios.put(
          `http://localhost:3000/meal/${editingId}`,
          formData,
          axiosConfig
        );
        Swal.fire({
    ...
    <>
      ...
      {openMealModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white rounded-lg shadow p-6 w-full max-w-2xl overflow-y-auto max-h-[90vh]">
            {/* Header */}
